import{setErrorAlert,resetForm,clearExistingGallery}from "./util.min.js";import{prepareGalleryCode}from "./codeSnippet.min.js";let scrollers=null;let scrollerInners=null;let numRows=1;const userRowsInput=document.getElementById("user-rows-input");const userRoundedSelect=document.getElementById("user-rounded-select");const userReverseSelect=document.getElementById("user-reverse-select");const userSlowdownSelect=document.getElementById("user-slowdown-select");const gallery=document.getElementById("gallery");let imageElements=null;let galleryCreated=!1;const createGalleryInput=document.getElementById("create-gallery-input");const userPreference=document.getElementById("user-preference");const userSubmitButton=document.getElementById("user-submit");const userImagesInput=document.getElementById("user-images-input");const errorAlert=document.getElementById("error-alert");const errorAlertMessage=document.getElementById("error-alert-message");const galleryCodeSection=document.getElementById("gallery-code-section");userSubmitButton.addEventListener("click",function(){handleImageUpload();numRows=userRowsInput.value;galleryCreated=!galleryCreated;resetForm({galleryCreated,createGalleryInput,userImagesInput,userRowsInput,userSubmitButton,userPreference,galleryCodeSection})});function handleImageUpload(){const files=userImagesInput.files;if(!galleryCreated&&!files.length){setErrorAlert({message:"Please upload at least 1 image.",errorAlert,errorAlertMessage});return}
let userImages=[];let promises=[];for(let i=0;i<files.length;i++){let filePromise=new Promise((resolve,reject)=>{const file=files[i];if(!file.type.startsWith("image/")){setErrorAlert({message:"Please upload only images.",errorAlert,errorAlertMessage});return}
